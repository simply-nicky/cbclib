...
NOINLINE static void calc_first_octant(size_t den, double *restrict res)
{
    ...
    for (size_t i = 1; i < l1; ++i)
        my_sincosm1pi((2.0 * i) / den, &res[2 * i]);
    size_t start = l1;
    while(start < n)
    {
        double cs[2];
        ...
        for (size_t i = 1; i < end; ++i)
        {
            ...
            res[2 * (start + i)] = ((cs[0] * csx[0] - cs[1] * csx[1] + cs[0]) + csx[0]) + 1.0;
...
NOINLINE static void calc_first_quadrant(size_t n, double *restrict res)
{
    ...
    size_t ndone = (n + 2) >> 2;
    size_t i, idx1, idx2;
    for (i = 0, idx1 = 0, idx2 = 2 * ndone - 2;
         i + 1 < ndone; i += 2, idx1 += 2, idx2 -= 2)
    {
        res[idx1]     = p[2 * i];
        res[idx1 + 1] = p[2 * i + 1];
        res[idx2]     = p[2 * i + 3];
...
NOINLINE static void calc_first_half(size_t n, double *restrict res)
{
    ...
    int i4, in, i;
    for (i4 = 0, in = n, i = 0; i4 <= in - i4; ++i, i4 += 4) // octant 0
    {
    ...
    if (ido == 1)
        ...
        for (size_t k = 0; k < l1; ++k)
        {
            ...
            for (size_t i = 1; i < ido; ++i)
            {
                ...
                cmplx cc0 = CC(i, 0, k), cc1 = CC(i, 1, k);
                cmplx cc2 = CC(i, 2, k), cc3 = CC(i, 3, k);
                PMC(t2, t1, cc0, cc2)
                PMC(t3, t4, cc1, cc3)
                ROT90(t4)
...
NOINLINE static int passg(size_t ido, size_t ip, size_t l1, cmplx *restrict cc, cmplx *restrict ch,
                          const cmplx *restrict wa, const cmplx *restrict csarr, const int sign)
{
    ...
    for (size_t l=1, lc=ip-1; l<ipph; ++l, --lc)
    {
        ...
        size_t iwal = 2 * l, j, jc;
        for (j = 3, jc = ip - 3; j < ipph - 1; j += 2, jc -= 2)
        {
...
int fft_np(void *plan, double complex *inp)
{
    if (!inp) return -1;
    if (!plan) return -1;
    int fail = cfft_forward((cfft_plan)plan, (double *)inp, 1.);
    return fail;
}
...
int ifft_np(void *plan, double complex *inp)
{
    if (!inp) return -1;
    if (!plan) return -1;
    int fail = cfft_backward((cfft_plan)plan, (double *)inp, 1.);
    return fail;
}
...
int rfft_np(void *plan, double *inp, size_t npts)
{
    if (!inp) return -1;
    if (!plan) return -1;
    memmove(inp + 1, inp, npts * sizeof(double));
    int fail = rfft_forward((rfft_plan)plan, inp + 1, 1.0);
    inp[0] = inp[1]; inp[1] = 0.0;
    if (!(npts % 2)) inp[npts + 1] = 0.0;
    return fail;
}
...
int irfft_np(void *plan, double *inp, size_t npts)
{
    if(!inp) return -1;
    if (!plan) return -1;
    memmove(inp + 1, inp + 2, npts * sizeof(double));
    int fail = rfft_backward((rfft_plan)plan, inp, 1.);
    return fail;
}
...
{
    return searchsorted(&key, LPRE, LPRE_SIZE, sizeof(size_t), SEARCH_LEFT, compare_size_t);
}
...
int fft_fftw(void *plan, double complex *inp)
{
    if (!inp) return -1;
    if (!plan) return -1;
    fftw_execute_dft((fftw_plan) plan, (fftw_complex *)inp, (fftw_complex *)inp);
    return 0;
}
...
int rfft_fftw(void *plan, double *inp, size_t npts)
{
    if (!inp) return -1;
    if (!plan) return -1;
    fftw_execute_dft_r2c((fftw_plan)plan, (double *)inp, (fftw_complex *)inp);
    return 0;
}
...
int irfft_fftw(void *plan, double *inp, size_t npts)
{
    if(!inp) return -1;
    if (!plan) return -1;
    fftw_execute_dft_c2r((fftw_plan)plan, (fftw_complex *)inp, (double *)inp);
    return 0;
}