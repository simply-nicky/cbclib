...
class CrystData(DataContainer):
    ...
    def update_background(self: C, effs: np.ndarray) -> C:
        """Return a new :class:`CrystData` object with a new set of backgrounds. A set of
        backgrounds is generated by fitting eigen flat-fields obtained from the Principal
        Component Analysis [PCA]_ to the measured data.

        Args:
            effs : Set of Eigen flat-fields used for the 'pca' generation method.
        ...
        See Also:
            :func:`cbclib.CrystData.get_pca` : Method to generate eigen flat-fields.
        """
        raise self._no_whitefield_exc
...
class CrystDataFull(CrystDataPart):
    ...
    def import_patterns(self, table: pd.DataFrame) -> CrystDataFull:
        ...
        for index, frame in zip(self.good_frames, self.frames[self.good_frames]):
            ...
            pattern['x'], pattern['y'] = self.forward_points(pattern['x'], pattern['y'])
...
class Detector(DataContainer):
    ...
    def export_table(self, dilation: float=0.0, profile: str='tophat',
    ...
    def export_table(self, dilation: float=0.0, profile: str='tophat',
        ...
        for idx, streaks in self.streaks.items():
            df = streaks.pattern_dataframe(shape=self.shape[1:], dilation=dilation,
                                           profile=profile, drop_duplicates=True)
...
class ModelDetector(Detector):
    ...
    def detect(self, q_abs: float=0.3, width: float=4.0, threshold: float=0.95, dp: float=1e-3):
        ...
        for idx, model in self.models.items():
            ...
            index = self.parent().good_frames[idx]
            streaks[idx] = model.filter_streaks(hkl=hkl, signal=self.parent().cor_data[index],
